...
function detectLanguage(text) {
    ...
    for (const word of words) {
        if (NEUTRAL_WORDS.has(word))
            continue; // Skip code/technical noise
        if (EN_HIGH.has(word))
            enScore++;
        else if (DE_HIGH.has(word))
            deScore++;
    }
    ...
    if (ratio < 1.5)
        return 'en'; // Weak signal → default to English
...
class Troglodyte {
    ...
    compress(prompt, options) {
        ...
        if (isTechnical) {
            if (debug)
                console.log('[Troglodyte] Smart Mode: Detected technical context. Reducing synonym replacement.');
